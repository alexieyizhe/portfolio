...
var getAction = function (v, _a, _b) {
    ...
    heyListen.invariant(animationLookup.has(type), "Invalid transition type '" + type + "'. Valid transition types are: tween, spring, decay, physics and keyframes.");
    if (type === 'tween') {
        var typeOfEase = typeof ease;
        if (typeOfEase !== 'function') {
            if (typeOfEase === 'string') {
                heyListen.invariant(easingLookup.has(ease), "Invalid easing type '" + ease + "'. popmotion.io/pose/api/transition");
...
var createDimensions = (function (element) {
    ...
    return {
        ...
        measurementAsPixels: function (measurement, value, type) {
            return type === styleValueTypes.percent && typeof value === 'number'
